...
class ThreadUsuario extends Thread {
  ...
  ThreadUsuario (Socket s, String host, String user, String password, JTextArea area) throws IOException{
  ...
  //Crea el fichero de logs del día anterior
  private void crearFichLog(){
    try {
      File fich = new File(".\\","logs-"+fechaHoy+".txt");
      FileOutputStream osFich = new FileOutputStream(fich);
      osFich.write(AreaInfo.getText().getBytes());
      osFich.flush();
      osFich.close();
      fechaHoy = getFecha();
      AreaInfo.setText("");
  ...
  private void terminar() throws IOException {
    ...
    if (datosUsuario!=null) {
      if (!getFecha().equals(fechaHoy))
        crearFichLog(); //Crea un nuevo fichero de logs
      AreaInfo.append("El usuario '"+datosUsuario.getNombreCompleto()+"' se ha desconectado.\n");
    }
          ...
          if (tipo.equalsIgnoreCase("login")){
            ...
            try {
              ...
              if ((datosUsuario=conBD.login(login, password))!=null) { //Comprobamos si existe el usuario, login correcto
                ...
                if (!getFecha().equals(fechaHoy))
                  crearFichLog(); //Crea un nuevo fichero de logs
                AreaInfo.append("El usuario '"+datosUsuario.getNombreCompleto()+"' se ha conectado al canal '"+canal.getNombre()+"'\n");
              } else
