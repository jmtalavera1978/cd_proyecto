<?php
	...
	global $autor_mensaje, $email_mensaje, $web_mensaje, $imagen_mensaje, $permitir_buscador_foro, $permitir_codigo_html;
	...
	if (@$HTTP_POST_VARS) {
		...
		$sql = "INSERT INTO mensajes (id_tema, id_padre, asunto, ";
		...
		$sql .= "autor_host, descripcion, fecha_publicacion) VALUES ('$id_tema', '$padre', '$asunto', ";
		if ($autor_mensaje) $sql .= "'$autor', ";
		...
		if ($padre!='0') {
			$sql = "UPDATE mensajes SET num_respuestas=num_respuestas+1, fecha_ultima_respuesta='$fecha' WHERE id_mensaje='$padre' AND id_tema='$id_tema'";
			consulta ($sql);
		}
		header ("Location: foro.php?id=$id_tema&titulo=$titulo&pag=$pag");
		exit;
	}
	$javascript = "
          ...
          <tr> 
            ...
            <td width="<?=$tam_enlaces?>"> <table width="100%" border="0" cellspacing="0" cellpadding="0">
				...
				<?php if ($permitir_iconos_como_enlaces) { ?>
					  ...
					  <td align="right"><a href='buscar.php?id=<?=$id_tema?>&titulo=<?=$titulo?>&pag=<?=$pag?>'><img src="img/buscar.gif" alt="Buscar" width="20" height="20"></a></td>
					  <?php } ?>
				 <?php } else { ?>
					  ...
					  <!-- foro.php?id=<?=$id_tema;?>&titulo=<?=$titulo?> -->
					  <?php if ($permitir_buscador_foro) { ?>
					  <td align="right"><a href='buscar.php?id=<?=$id_tema?>&titulo=<?=$titulo?>&pag=<?=$pag?>'>Buscar</a></td>
					  <?php } ?>
				 <?php } ?>
                </tr>
              </table></td>
          ...
          <table width="100%" border="0" cellspacing="5" cellpadding="0" class="tabla_de_temas">
            <tr> 
              <td><table width="100%" border="0" cellspacing="5" cellpadding="0">
                  <tr> 
                    <td width="30%" class="campo_oblig"><div align="right">Asunto: </div></td>
				  ...
				  <tr> 
                    <td><div align="right"><font class="campo">Autor: </font></div></td>
                    <td><input name="autor" type="text" class="datos_form" size="50" maxlength="255"></td>
                  </tr>
				  <?php } if ($email_mensaje) {?>
                  <tr> 
                    <td><div align="right"><font class="campo">E-mail: </font></div></td>
                    <td><input name="email" type="text" class="datos_form" size="50" maxlength="255"></td>
                  </tr>
				  <?php } if ($web_mensaje) { ?>
                  <tr> 
                    <td><div align="right"><font class="campo">Web: </font></div></td>
                    <td><input name="web" type="text" class="datos_form" size="50" maxlength="255" onBlur="comprobarHTTP(document.mensaje.web);"></td>
                  </tr>
				  <?php }?>
                  <tr> 
