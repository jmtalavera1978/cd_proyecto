          ...
          <tr> 
            ...
            <td width="<?=$tam_enlaces?>"> <table width="100%" border="0" cellspacing="0" cellpadding="0">
				...
				<?php if ($permitir_iconos_como_enlaces) { ?>
                  	<td><a href="foro<?php if (@$tipo=="privado") echo "_privado"; else if (@$tipo=="protegido") echo "_protegido"; ?>.php?id=<?=$id_tema;?>&titulo=<?=$titulo?>&pag=<?=$pag?>&filtro=<?=$filtrar?>&orden=<?=$orden?>"><img src="img/volver1.gif" alt=" Volver al foro" width="20" height="20"></a></td>
                  	<td align="right"><a href='buscar.php?id=<?=$id_tema?>&titulo=<?=$titulo?>&pag=<?=$pag?>&filtro=<?=$filtrar?>&orden=<?=$orden?>&tipo=<?=@$tipo?>'><img src="img/buscar.gif" alt="Nueva b&uacute;squeda" width="20" height="20"></a></td>
				<?php } else { ?>
					<td><a href="foro<?php if (@$tipo=="privado") echo "_privado"; else if (@$tipo=="protegido") echo "_protegido"; ?>.php?id=<?=$id_tema;?>&titulo=<?=$titulo?>&pag=<?=$pag?>&filtro=<?=$filtrar?>&orden=<?=$orden?>">Volver al foro</a></td>
                  	<td align="right"><a href='buscar.php?id=<?=$id_tema?>&titulo=<?=$titulo?>&pag=<?=$pag?>&filtro=<?=$filtrar?>&orden=<?=$orden?>&tipo=<?=@$tipo?>'>Nueva b&uacute;squeda</a></td>
				<?php } ?>
                </tr>
              </table></td>
          </tr>
    ...
    <tr>
      <td>
          <table width="100%" border="0" cellspacing="5" cellpadding="0" class="tabla_de_temas">
            <tr> 
              <td><form method="POST" action="<?=$PHP_SELF;?>?id=<?=$id_tema;?>&titulo=<?=$titulo?>&pag=<?=$pag?>&filtro=<?=$filtrar?>&orden=<?=$orden?>&tipo=<?=$tipo?>" enctype="multipart/form-data"><br>
...
<?php
	...
	if ($busqueda) {
		...
		//CREAMOS LA SENTENCIA SQL DE LA BÚSQUEDA según el tipo de foro de la búsqueda Y MOSTRAMOS LOS RESULTADOS
		$sql = "SELECT id_mensaje, asunto, ";
		if ($tipo=="privado" || $tipo=="protegido")
			$sql.= " usuario, ";
		else
			$sql.= " autor, "; 
		$sql .= " fecha_publicacion, lecturas, num_respuestas, fecha_ultima_respuesta ";
		if ($tipo=="privado")
			$sql.= " FROM mensajes_privados ";
		else if ($tipo=="protegido")
			$sql.= " FROM mensajes_protegidos ";
		else
			$sql.= " FROM mensajes ";
