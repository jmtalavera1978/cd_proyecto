<?php
	...
	if (@$HTTP_POST_VARS) {
		...
		$sql = "INSERT INTO mensajes_privados (id_tema, id_padre, asunto, usuario, ";
		...
		if ($padre!='0') {
			$sql = "UPDATE mensajes_privados SET num_respuestas=num_respuestas+1, fecha_ultima_respuesta='$fecha' WHERE id_mensaje='$padre' AND id_tema='$id_tema'";
			consulta ($sql);
		}
		...
		if ($permitir_moderacion_foro)
			echo "	alert ('Su mensaje será publicado en breve cuando sea aceptado por el administrador');";
		echo "	document.location='foro_privado.php?id=$id_tema&titulo=$titulo&pag=$pag';\n";
		echo "</script>\n";
		exit;
	}
	$javascript = "
          ...
          <tr> 
            ...
            <td width="<?=$tam_enlaces?>"> <table width="100%" border="0" cellspacing="0" cellpadding="0">
				...
				<?php if ($permitir_iconos_como_enlaces) { ?>
					  ...
					  <td align="right"><a href='buscar.php?id=<?=$id_tema?>&titulo=<?=$titulo?>&pag=<?=$pag?>&tipo=privado'><img src="img/buscar.gif" alt="Buscar" width="20" height="20"></a></td>
					  <?php } ?>
				 <?php } else { ?>
					  ...
					  <!-- foro.php?id=<?=$id_tema;?>&titulo=<?=$titulo?> -->
					  <?php if ($permitir_buscador_foro) { ?>
					  <td align="right"><a href='buscar.php?id=<?=$id_tema?>&titulo=<?=$titulo?>&pag=<?=$pag?>&tipo=privado'>Buscar</a></td>
					  <?php } ?>
				 <?php } ?>
                </tr>
              </table></td>
