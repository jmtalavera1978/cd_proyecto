...
function listar_mensajes_tabla ($res, $id_tema, $titulo, $pag, $filtro, $orden, $es_acceso_administrador)
{
	...
	<?php
		while ($reg = @mysql_fetch_array($res)) {
			...
			if ($es_acceso_administrador) echo "            <td align=\"center\"><a href=\"index.php?borrar=mensaje_publico&id=$id_mensaje&id_tema=$id_tema\">Borrar mensaje</a></td>\n";
			echo "          </tr>\n";
		}
	} else {
...
/****************************************************
 La siguiente función escribe la tabla formateada de
 una consulta de un conjunto de mensajes protegidos del foro.
 ****************************************************/
function listar_mensajes_protegidos($sql, $id_tema, $titulo, $pag, $filtro, $orden, $es_acceso_administrador) {
	global $formato_foro;
	...
	if ($formato_foro == "tabla" || $es_acceso_administrador) {
		listar_mensajes_tabla_prot ($res, $id_tema, $titulo, $pag, $filtro, $orden, $es_acceso_administrador);
	} else if ($formato_foro== "lista") {
		listar_mensajes_lista_prot ($res, $id_tema, $titulo, $pag, $filtro, $orden);
	}
	@mysql_free_result($res);
	echo "	</td></tr>\n";
}

function listar_mensajes_tabla_prot ($res, $id_tema, $titulo, $pag, $filtro, $orden, $es_acceso_administrador)
{
	global $autor_mensaje;
	if (@mysql_num_rows($res)>0) {
	?>
	...
	<?php
		while ($reg = @mysql_fetch_array($res)) {
			$id_mensaje = $reg ['id_mensaje'];
			$asunto = htmlentities($reg ['asunto']);
			$autor = htmlentities($reg ['usuario']);
			$fecha_publicacion = $reg ['fecha_publicacion'];
			$lecturas = $reg ['lecturas'];
			$num_respuestas = $reg ['num_respuestas'];
			$fecha_ult_resp = $reg ['fecha_ultima_respuesta'];
			...
			if ($es_acceso_administrador)
				...
				echo "            <td><a href=\"mensaje_protegido.php?id=$id_tema&id_mensaje=$id_mensaje&titulo=$titulo&pag=$pag&filtro=$filtro&orden=$orden\"><img src=\"img/folder.gif\" alt=\"Leer mensaje\" width=\"15\" heigth=\"15\"> $asunto</a></td>\n";
			echo "            <td>$autor</td>\n";
			echo "            <td align=\"center\">".formatear_fecha($fecha_publicacion)."</td>\n";
			echo "            <td align=\"center\">$lecturas</td>\n";
			echo "            <td align=\"center\">$num_respuestas</td>\n";
			...
			if ($es_acceso_administrador) echo "            <td align=\"center\"><a href=\"index.php?borrar=mensaje_protegido&id=$id_mensaje&id_tema=$id_tema\">Borrar mensaje</a></td>\n";
			echo "          </tr>\n";
		}
	} else {
		echo "<table width=\"100%\" border=\"0\" cellspacing=\"2\" cellpadding=\"1\" class=\"tabla_de_temas\"><tr class=\"fila_de_tabla\"><td>No hay mensajes</td></tr></table>\n";
...
function listar_mensajes_lista_prot ($res, $id_tema, $titulo, $pag, $filtro, $orden)
{
	global $autor_mensaje;
	//$indice = (($pag -1) * $mensajes_x_pag) + 1;
	if (@mysql_num_rows($res)>0) {
	...
	<?php
		while ($reg = @mysql_fetch_array($res)) {
			...
			//escribo mensaje y...listo los hijos del nivel 1
			echo "<table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\" class=\"tabla_de_temas_lista\">\n";
			echo "<tr class=\"fila_de_tabla\"><td><a href=\"mensaje_protegido.php?id=$id_tema&id_mensaje=$id_mensaje&titulo=$titulo&pag=$pag&filtro=$filtro&orden=$orden\"><img src=\"img/folder.gif\" alt=\"Leer mensaje\" width=\"15\" heigth=\"15\"> $asunto</a></td>\n";
			echo "<td align=\"right\">".formatear_fecha($fecha_publicacion)."</td></tr>\n"; //, por $autor ($lecturas lecturas)
			echo "</table>\n";
			
			$matriz = array(); //Matriz de booleanos que indica donde sigue el camino de directorios
			listar_mensajes_lista_hijos_prot($id_mensaje, $id_tema, $titulo, $pag, 1, $matriz, $filtro, $orden);
		}
	} else {
		echo "<table width=\"100%\" border=\"0\" cellspacing=\"2\" cellpadding=\"1\" class=\"tabla_de_temas\"><tr class=\"fila_de_tabla\"><td>No hay mensajes</td></tr></table>\n";
	}
...
function listar_mensajes_lista_hijos_prot ($id_padre, $id_tema, $titulo, $pag, $nivel, $matriz, $filtro, $orden)
{
	global $autor_mensaje;
	$res = consulta ("select * from mensajes where id_tema='$id_tema' and id_padre='$id_padre' order by fecha_publicacion");
	...
	if (@mysql_num_rows($res)>0) {
		...
		while ($reg = @mysql_fetch_array($res)) {
			...
			echo "<td><a href=\"mensaje_protegido.php?id=$id_tema&id_mensaje=$id_mensaje&titulo=$titulo&pag=$pag&filtro=$filtro&orden=$orden\"><img src=\"img/carpeta.gif\" alt=\"Leer mensaje\" width=\"15\" heigth=\"15\"> $asunto</a></td>\n";
			echo "<td align=\"right\">".formatear_fecha($fecha_publicacion)."</td></tr>\n"; //, por $autor ($lecturas lecturas)
			echo "</table>\n";
			listar_mensajes_lista_hijos_prot($id_mensaje, $id_tema, $titulo, $pag, $nivel+1, $matriz, $filtro, $orden);
		}
	}
}
...
/****************************************************
 La siguiente función escribe la tabla formateada de
 una consulta de un conjunto de mensajes  privados del foro.
 ****************************************************/
function listar_mensajes_privados($sql, $id_tema, $titulo, $pag, $filtro, $orden, $es_acceso_administrador) {
	global $formato_foro;
	...
	if ($formato_foro == "tabla" || $es_acceso_administrador) {
		listar_mensajes_tabla_priv ($res, $id_tema, $titulo, $pag, $filtro, $orden, $es_acceso_administrador);
	} else if ($formato_foro== "lista") {
		listar_mensajes_lista_priv ($res, $id_tema, $titulo, $pag, $filtro, $orden);
	}
	@mysql_free_result($res);
	echo "	</td></tr>\n";
}

function listar_mensajes_tabla_priv ($res, $id_tema, $titulo, $pag, $filtro, $orden, $es_acceso_administrador)
{
	global $autor_mensaje, $permitir_moderacion_foro;
	if (@mysql_num_rows($res)>0) {
	?>
    	<table width="100%" border="0" cellspacing="2" cellpadding="1" class="tabla_de_temas">
          <tr class="encabezado_de_temas">
		  ...
		  <?php } ?>
            ...
            <td align="center">&Uacute;LTIMA RESP.</td>
			<?php 
			if ($es_acceso_administrador && $permitir_moderacion_foro) echo "            <td align=\"center\">PUBLICADO</td>\n";
			if ($es_acceso_administrador) echo "            <td align=\"center\">&nbsp;</td>\n";
			?>
          </tr>
	<?php
		while ($reg = @mysql_fetch_array($res)) {
			...
			$num_respuestas = $reg ['num_respuestas'];
			$fecha_ult_resp = $reg ['fecha_ultima_respuesta'];
			if ($es_acceso_administrador && $permitir_moderacion_foro)
				$visible = $reg ['visible'];
			echo "			<tr class=\"fila_de_tabla\">\n";
			if ($es_acceso_administrador) echo "            <td align=\"center\">$id_mensaje</td>\n";
			if ($es_acceso_administrador)
				...
				echo "            <td><a href=\"mensaje_privado.php?id=$id_tema&id_mensaje=$id_mensaje&titulo=$titulo&pag=$pag&filtro=$filtro&orden=$orden\"><img src=\"img/folder.gif\" alt=\"Leer mensaje\" width=\"15\" heigth=\"15\"> $asunto</a></td>\n";
			echo "            <td>$autor</td>\n";
			echo "            <td align=\"center\">".formatear_fecha($fecha_publicacion)."</td>\n";
			echo "            <td align=\"center\">$lecturas</td>\n";
			echo "            <td align=\"center\">$num_respuestas</td>\n";
			...
			if ($es_acceso_administrador && $permitir_moderacion_foro) {
				if ($visible == 'SI')
					echo "            <td align=\"center\">SI (<a href=\"index.php?id_tema=$id_tema&publicarM=NO&id=$id_mensaje&foro=privado\">Ocultar</a>)</td>\n";
				else
					echo "            <td align=\"center\">NO (<a href=\"index.php?id_tema=$id_tema&publicarM=SI&id=$id_mensaje&foro=privado\">Visible</a>)</td>\n";
			}
			if ($es_acceso_administrador) echo "            <td align=\"center\"><a href=\"index.php?borrar=mensaje_privado&id=$id_mensaje&id_tema=$id_tema\">Borrar mensaje</a></td>\n";
			echo "          </tr>\n";
		}
	} else {
		echo "<table width=\"100%\" border=\"0\" cellspacing=\"2\" cellpadding=\"1\" class=\"tabla_de_temas\"><tr class=\"fila_de_tabla\"><td>No hay mensajes</td></tr></table>\n";
...
function listar_mensajes_lista_priv ($res, $id_tema, $titulo, $pag, $filtro, $orden)
{
	global $autor_mensaje;
	//$indice = (($pag -1) * $mensajes_x_pag) + 1;
	if (@mysql_num_rows($res)>0) {
	...
	<?php
		while ($reg = @mysql_fetch_array($res)) {
			...
			//escribo mensaje y...listo los hijos del nivel 1
			echo "<table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\" class=\"tabla_de_temas_lista\">\n";
			echo "<tr class=\"fila_de_tabla\"><td><a href=\"mensaje_privado.php?id=$id_tema&id_mensaje=$id_mensaje&titulo=$titulo&pag=$pag&filtro=$filtro&orden=$orden\"><img src=\"img/folder.gif\" alt=\"Leer mensaje\" width=\"15\" heigth=\"15\"> $asunto</a></td>\n";
			echo "<td align=\"right\">".formatear_fecha($fecha_publicacion)."</td></tr>\n"; //, por $autor ($lecturas lecturas)
			echo "</table>\n";
			
			$matriz = array(); //Matriz de booleanos que indica donde sigue el camino de directorios
			listar_mensajes_lista_hijos_priv($id_mensaje, $id_tema, $titulo, $pag, 1, $matriz, $filtro, $orden);
		}
	} else {
		echo "<table width=\"100%\" border=\"0\" cellspacing=\"2\" cellpadding=\"1\" class=\"tabla_de_temas\"><tr class=\"fila_de_tabla\"><td>No hay mensajes</td></tr></table>\n";
	}
...
function listar_mensajes_lista_hijos_priv ($id_padre, $id_tema, $titulo, $pag, $nivel, $matriz, $filtro, $orden)
{
	global $autor_mensaje, $permitir_moderacion_foro;
	
	if ($permitir_moderacion_foro)
		$res = consulta ("select * from mensajes where id_tema='$id_tema' and id_padre='$id_padre' and visible='SI' order by fecha_publicacion");
	else
		$res = consulta ("select * from mensajes where id_tema='$id_tema' and id_padre='$id_padre' order by fecha_publicacion");
	...
	if (@mysql_num_rows($res)>0) {
		...
		while ($reg = @mysql_fetch_array($res)) {
			...
			echo "<td><a href=\"mensaje_privado.php?id=$id_tema&id_mensaje=$id_mensaje&titulo=$titulo&pag=$pag&filtro=$filtro&orden=$orden\"><img src=\"img/carpeta.gif\" alt=\"Leer mensaje\" width=\"15\" heigth=\"15\"> $asunto</a></td>\n";
			echo "<td align=\"right\">".formatear_fecha($fecha_publicacion)."</td></tr>\n"; //, por $autor ($lecturas lecturas)
			echo "</table>\n";
			listar_mensajes_lista_hijos_priv($id_mensaje, $id_tema, $titulo, $pag, $nivel+1, $matriz, $filtro, $orden);
		}
	}
}
...
/****************************************************
 La siguiente función escribe la tabla formateada de
 una consulta del conjunto de temas de los foros públicos.
 ****************************************************/
function listar_foros($es_acceso_administrador) {
	...
	?>
	<table width="100%" border="0" cellspacing="2" cellpadding="1" class="tabla_de_temas">
          <tr class="encabezado_de_temas_menor">
		    <?php if ($es_acceso_administrador) echo "            <td align=\"center\">ID.</td>\n"; ?>
            <td width="25%">LISTA TEM&Aacute;TICA DE FOROS</td>
	...
	<?php
		while ($reg = @mysql_fetch_array($res)) {
			...
			if ($es_acceso_administrador)
				echo "            <td><a href=\"index.php?id_tema=$id_tema&foro=publico\">".htmlentities($titulo_tema)."</a></td>\n";
			else
			...
			if ($es_acceso_administrador) echo "            <td align=\"center\"><a href=\"index.php?borrar=foro_publico&id=$id_tema\">borrar foro</a></td></tr>\n";
			else echo "            </tr>\n";
		}
		echo "</table>\n";
	} else {
...
/****************************************************
 La siguiente función escribe la tabla formateada de
 una consulta del conjunto de temas de los foros privados.
 ****************************************************/
function listar_foros_privados($es_acceso_administrador) {
	global $autor_mensaje, $permitir_moderacion_foro;
	$sql = "SELECT * FROM temas_privados";
	if (!$es_acceso_administrador && $permitir_moderacion_foro) $sql .= " WHERE visible='SI'";
	$res = consulta ($sql);
	...
	<?php
		while ($reg = @mysql_fetch_array($res)) {
			...
			$titulo_tema = $reg ['titulo_tema'];
			$descripcion = $reg ['descripcion'];
			if ($es_acceso_administrador && $permitir_moderacion_foro) $visible = $reg ['visible'];
			$sql = "SELECT usuario, ";
			$sql .= "fecha_publicacion FROM mensajes_privados WHERE ";
			if (!$es_acceso_administrador && $permitir_moderacion_foro)
				$sql .= " visible='SI' AND ";
			$sql .= "id_tema = '$id_tema' ORDER BY fecha_publicacion DESC"; //AND id_padre='0'
			$res_num_mensajes = consulta ($sql); 
			$num_mensajes = @mysql_num_rows($res_num_mensajes);
			if ($num_mensajes>0) {
				$reg2 = @mysql_fetch_array ($res_num_mensajes);
				if ($autor_mensaje)  {
					$ultimo_autor = $reg2['usuario'];
					if (!$ultimo_autor) $ultimo_autor = "An&oacute;nimo";
				}
				$fecha_ultima_publicacion = $reg2['fecha_publicacion'];
			}
			...
			if ($es_acceso_administrador) echo "            <td align=\"center\">$id_tema</td>\n";
			if ($es_acceso_administrador)
				echo "            <td><a href=\"index.php?id_tema=$id_tema&foro=privado\">".htmlentities($titulo_tema)."</a></td>\n";
			else
				echo "            <td><a href=\"foro_privado.php?id=$id_tema&titulo=$titulo_tema&pag=1\">".htmlentities($titulo_tema)."</a></td>\n";
			echo "            <td>".htmlentities($descripcion)."</td>\n";
			if ($num_mensajes)
				echo "            <td align=\"center\">$num_mensajes</td>\n";
			else
			...
			if ($es_acceso_administrador && $permitir_moderacion_foro) {
				if ($visible == 'SI')
					echo "            <td align=\"center\">SI (<a href=\"index.php?publicar=NO&id=$id_tema\">Ocultar</a>)</td>\n";
				else
					echo "            <td align=\"center\">NO (<a href=\"index.php?publicar=SI&id=$id_tema\">Visible</a>)</td>\n";
			}
			if ($es_acceso_administrador) echo "            <td align=\"center\"><a href=\"index.php?borrar=foro_privado&id=$id_tema\">borrar foro</a></td></tr>\n";
			else echo "            </tr>\n";
		}
		echo "</table>\n";
	} else {
...
/****************************************************
 La siguiente función escribe la tabla formateada de
 una consulta del conjunto de temas de los foros protegidos.
 ****************************************************/
function listar_foros_protegidos($es_acceso_administrador) {
	global $autor_mensaje;
	$res = consulta ("select * from temas_protegidos");
	...
	<?php
		while ($reg = @mysql_fetch_array($res)) {
			...
			$descripcion = $reg ['descripcion'];
			$sql = "SELECT usuario, ";
			$sql .= "fecha_publicacion FROM mensajes_protegidos WHERE id_tema = '$id_tema' ORDER BY fecha_publicacion DESC"; //AND id_padre='0'
			$res_num_mensajes = consulta ($sql); 
			$num_mensajes = @mysql_num_rows($res_num_mensajes);
			if ($num_mensajes>0) {
				$reg2 = @mysql_fetch_array ($res_num_mensajes);
			...
			if ($es_acceso_administrador) echo "            <td align=\"center\">$id_tema</td>\n";
			if ($es_acceso_administrador)
				echo "            <td><a href=\"index.php?id_tema=$id_tema&foro=protegido\">".htmlentities($titulo_tema)."</a></td>\n";
			else
				echo "            <td><a href=\"foro_protegido.php?id=$id_tema&titulo=$titulo_tema&pag=1\">".htmlentities($titulo_tema)."</a></td>\n";
			echo "            <td>".htmlentities($descripcion)."</td>\n";
			if ($num_mensajes)
				echo "            <td align=\"center\">$num_mensajes</td>\n";
			else
			...
			if ($num_mensajes>0) {
				...
				echo "            <td align=\"center\">No hay mensajes</td>\n";
			if ($es_acceso_administrador) echo "            <td align=\"center\"><a href=\"index.php?borrar=foro_protegido&id=$id_tema\">borrar foro</a></td></tr>\n";
			else echo "            </tr>\n";
		}
		echo "</table>\n";
	} else {
