<?php
	...
	if (!session_is_registered('login_usuario')) {
		...
		echo "	document.location='foro.php';\n";
		echo "</script>\n";
		exit;
	}
	...
	if ($id_mensaje) {
		if ($permitir_valoraciones_foro) {
			...
			if (@$HTTP_POST_VARS['voto']) {
				...
				$sql = "UPDATE mensajes_privados SET num_valoraciones='$num_v'+1, valoracion_total='$valoracion_total' 
						WHERE id_mensaje='$id_mensaje' AND id_tema='$id_tema'";
				consulta($sql);
			} else {
				//En caso contrario, actualizamos las lecturas de este mensaje
				consulta("UPDATE mensajes_privados SET lecturas=lecturas+1 WHERE id_mensaje='$id_mensaje' AND id_tema='$id_tema'");
			}
		} else {
			//En caso contrario, actualizamos las lecturas de este mensaje
			consulta("UPDATE mensajes_privados SET lecturas=lecturas+1 WHERE id_mensaje='$id_mensaje' AND id_tema='$id_tema'");
		...
		$sql = "SELECT id_mensaje, id_padre, num_respuestas, asunto, usuario, ";
		if ($imagen_mensaje) $sql .= " imagen_name,";
		if ($permitir_valoraciones_foro) $sql .= " valoracion_total, num_valoraciones,";
		$sql .= "descripcion, fecha_publicacion, lecturas FROM mensajes_privados WHERE id_mensaje='$id_mensaje' AND id_tema='$id_tema'";
		$res = consulta ($sql);
	}
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
          ...
          <td width="<?=$tam_enlaces?>"> <table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <?php if ($permitir_iconos_como_enlaces) { ?>
					...
					?>
					<td><div align="center"><a href="foro_privado.php?id=<?=$id_tema;?>&titulo=<?=$titulo?>&pag=<?=$pag?>&filtro=<?=$filtro?>&orden=<?=$orden?>"><img src="img/volver2.gif" alt="Volver al foro" width="20" height="20"></a></div></td>
					<td align="right"><a href='escribir_privado.php?id=<?=$id_tema?>&titulo=<?=$titulo?>&padre=<?=$id_mensaje?>&asunto=<?=$asunto?>&pag=<?=$pag?>'><img src="img/responder.gif" alt="Responder" width="20" height="20"></a></td>
				<?php } else { ?>
						  <?php if (@$id_padre!="0") {
							echo "                  <td><a href=\"javascript:history.back();\">Volver atr&aacute;s</a></td>\n"; //mensaje.php?id=$id_tema&id_mensaje=$id_padre&titulo=$titulo
						  } 
					?>
					<td><div align="center"><a href="foro_privado.php?id=<?=$id_tema;?>&titulo=<?=$titulo?>&pag=<?=$pag?>&filtro=<?=$filtro?>&orden=<?=$orden?>">Volver 
						al foro</a></div></td>
					<td align="right"><a href='escribir_privado.php?id=<?=$id_tema?>&titulo=<?=$titulo?>&padre=<?=$id_mensaje?>&asunto=<?=$asunto?>&pag=<?=$pag?>'>Responder</a></td>
				<?php } ?>
              </tr>
            </table></td>
          </tr>
            ...
            <td height="18"> 
              <?php
			  		if ($reg) {
						...
						$res_autor = consulta("SELECT nombre_usuario, apellidos_usuario, email_usuario, web_usuario FROM usuarios WHERE login='$autor'");
						$reg_autor = @mysql_fetch_array($res_autor);
						$nombre_usuario = $reg_autor['nombre_usuario']." ".$reg_autor['apellidos_usuario'];
						$email = $reg_autor['email_usuario'];
						$web = $reg_autor['web_usuario'];
...
<?php
	...
	$sql = "SELECT id_mensaje, asunto, usuario, fecha_publicacion, lecturas, num_respuestas, fecha_ultima_respuesta 
			FROM mensajes_privados WHERE id_padre='$id_mensaje' AND id_tema='$id_tema' ORDER BY fecha_publicacion DESC";

	//Creamos la tabla de respuestas del mensaje
	listar_mensajes_privados($sql, $id_tema, $titulo, $pag, $filtro, $orden, false);
