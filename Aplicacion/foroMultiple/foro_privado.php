<?php
	...
	global $autor_mensaje, $formato_foro, $permitir_filtro_foro, $permitir_varios_foros, $permitir_paginado_foro, $permitir_moderacion_foro,
			$num_mensajes_x_pagina_foro, $permitir_buscador_foro, $mostrar_usuarios_conectados_foro;

	if (!session_is_registered('login_usuario') && $permitir_varios_foros) {
			  ...
			  <table width="100%" border="0" cellspacing="0" cellpadding="0">
				...
				<?php if ($permitir_iconos_como_enlaces) { ?>
					<?php if ($permitir_varios_foros) { ?>
				  	...
				  	<?php } ?>
                  		<td align="<?php if ($permitir_buscador_foro) echo "center"; else echo "right"; ?>"><a href="escribir_privado.php?id=<?=$id_tema;?>&titulo=<?=$titulo?>&padre=0"><img src="img/nuevo.gif" alt="Nuevo mensaje" width="20" height="20"></a></td>
				  	<?php if ($permitir_buscador_foro) { ?>
                  		<td align="right"><a href='buscar.php?id=<?=$id_tema?>&titulo=<?=$titulo?>&pag=<?=$pag?>&filtro=<?=$filtro?>&orden=<?=$orden?>&tipo=privado'><img src="img/buscar.gif" alt="Buscar" width="20" height="20"></a></td>
				  <?php } ?>
				<?php } else { ?>
					<?php if ($permitir_varios_foros) { ?>
                  		<td><a href="index.php">Lista tem&aacute;tica</a></td>
				  	<?php } ?>
                  		<td align="<?php if ($permitir_buscador_foro) echo "center"; else echo "right"; ?>"><a href="escribir_privado.php?id=<?=$id_tema;?>&titulo=<?=$titulo?>&padre=0">Nuevo mensaje</a></td>
				  	<?php if ($permitir_buscador_foro) { ?>
                  		<td align="right"><a href='buscar.php?id=<?=$id_tema?>&titulo=<?=$titulo?>&pag=<?=$pag?>&filtro=<?=$filtro?>&orden=<?=$orden?>&tipo=privado'>Buscar</a></td>
				  <?php } ?>
				<?php } ?>
                </tr>
              </table></td>
	...
	<tr>
      <td class="encabezado_de_foro">
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
          ...
          <form name="filtrar" action="foro_privado.php?id=<?=$id_tema?>&titulo=<?=$titulo?>&pag=<?=$pag?>" enctype="multipart/form-data" method="post">
...
<?php
	...
	if ($permitir_paginado_foro) {
		...
		$sql = "select id_mensaje, asunto, fecha_publicacion, lecturas, num_respuestas, fecha_ultima_respuesta from mensajes_privados where id_tema='$id_tema' and id_padre='0'";
		if ($permitir_moderacion_foro) $sql .= " AND visible='SI'";
	...
	$sql = "SELECT id_mensaje, asunto, usuario, fecha_publicacion, lecturas, num_respuestas, fecha_ultima_respuesta FROM mensajes_privados WHERE id_tema='$id_tema' AND id_padre='0'";
	if ($permitir_moderacion_foro) $sql .= " AND visible='SI'";
