...
function listar_mensajes_tabla ($res, $id_tema, $titulo, $pag, $filtro, $orden, $es_acceso_administrador)
{
	...
	<?php
		while ($reg = @mysql_fetch_array($res)) {
			...
			if ($es_acceso_administrador && $permitir_moderacion_foro) {
				if ($visible == 'SI')
					echo "            <td align=\"center\">SI (<a href=\"index.php?id_tema=$id_tema&publicarM=NO&id=$id_mensaje\">Ocultar</a>)</td>\n";
				else
					echo "            <td align=\"center\">NO (<a href=\"index.php?id_tema=$id_tema&publicarM=SI&id=$id_mensaje\">Visible</a>)</td>\n";
			}
...
function listar_foros($es_acceso_administrador) {
	...
	<?php
		while ($reg = @mysql_fetch_array($res)) {
			...
			$sql = "SELECT usuario, ";
			$sql .= "fecha_publicacion FROM mensajes WHERE";
			if (!$es_acceso_administrador && $permitir_moderacion_foro)
				$sql .= " visible='SI' AND ";
